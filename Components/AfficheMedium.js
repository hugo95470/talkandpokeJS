...
export default function AfficheMedium(props) {
    ...
    let ItemAffiche = ({ code, date, imageSup, afficheTitre, AfficheId, AfficheTitreSup }) => {  
        ...
        return(
            <TouchableOpacity activeOpacity={1} onPress={() => props.navigation.navigate('DetailsOeuvrePage', {AfficheId: AfficheId, _Image: code})}>
                <View style={[globalStyles.shadows, {margin: 3}]}>

                        <ImageBackground isBackground imageStyle={{ borderRadius: 15}} source={afficheImage[code]} resizeMode="cover" style={[styles.affiche, globalStyles.borderRadius]}>
                            ...
                            <TouchableOpacity style={{position: 'absolute', bottom: 7, left: 7, right: 10,}} activeOpacity={1} onPress={() => props.navigation.navigate('MessagePage', {_isOeuvre: true, _oeuvreId: AfficheId})}>
    ...
    return(
        <ItemAffiche afficheTitre={props.item.AfficheTitre} code={props.item.Code} date={props.item.CreateDate} imageSup={props.item.ImageSup} AfficheTitreSup={props.item.AfficheTitreSup} AfficheId={props.item.AfficheId}/>
    ) 
